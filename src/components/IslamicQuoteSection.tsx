...
export default function IslamicQuoteSection() {
  ...
  return (
    <section className="py-14 bg-stone-50 text-stone-900 border-y border-gold-500/10 relative overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 left-0 flex items-center justify-center opacity-5 pointer-events-none">
        <svg className="w-96 h-96 text-gold-500" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 relative z-10 text-center">
        <HeartHandshake className="w-8 h-8 text-gold-600 mx-auto mb-4 animate-pulse" />
        ...
        <div className="relative h-28 sm:h-24 flex items-center justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="space-y-3"
            >
              <blockquote className="font-serif text-sm sm:text-base md:text-lg italic leading-relaxed text-stone-850">
                {activeQuote.quote[language]}
              </blockquote>
              <cite className="block text-xs font-semibold uppercase tracking-widest text-gold-600 font-sans not-italic">
        ...
        <div className="flex items-center justify-center gap-1.5 mt-6">
          {ISLAMIC_QUOTES.map((_, i) => (
            <button
              key={i}
              onClick={() => setIndex(i)}
              className={`h-1 rounded-sm transition-all cursor-pointer ${i === index ? 'w-6 bg-gold-500' : 'w-2 bg-stone-200'}`}
            />
          ))}
        </div>
